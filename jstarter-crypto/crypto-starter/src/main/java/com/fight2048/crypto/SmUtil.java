...
import com.fight2048.crypto.asymmetric.SM2;
import com.fight2048.crypto.digest.HMac;
import com.fight2048.crypto.digest.HmacAlgorithm;
import com.fight2048.crypto.digest.SM3;
import com.fight2048.crypto.digest.mac.BCHMacEngine;
import com.fight2048.crypto.digest.mac.MacEngine;
import com.fight2048.crypto.symmetric.SM4;
import com.fight2048.crypto.symmetric.SymmetricCrypto;
...
public class SmUtil {
    ...
    public static byte[] rsAsn1ToPlain(byte[] rsDer) throws IOException {
        final BigInteger[] decode = StandardDSAEncoding.INSTANCE.decode(SM2_DOMAIN_PARAMS.getN(), rsDer);
        final byte[] r = bigIntToFixedLengthBytes(decode[0]);
        final byte[] s = bigIntToFixedLengthBytes(decode[1]);

        final byte[] result = new byte[r.length + s.length];
